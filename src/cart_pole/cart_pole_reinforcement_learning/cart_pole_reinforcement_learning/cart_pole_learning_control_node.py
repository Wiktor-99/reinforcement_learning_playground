...
class CartPoleReinforcementLearning(Node):
    ...
    def update_simulation_status(self):
        self.is_truncated = np.isclose(
            self.cart_observations[2], self.UPPER_POLE_LIMIT, rtol=1e-05, atol=1e-08, equal_nan=False
        ) or np.isclose(self.cart_observations[2], self.LOWER_POLE_LIMIT, rtol=1e-05, atol=1e-08, equal_nan=False)
